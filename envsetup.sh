...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^axxion_") ; then
       axxion_BUILD=$(echo -n $1 | sed -e 's/^axxion_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $axxion_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       axxion_BUILD=
    fi
    export axxion_BUILD
...
function setpaths()
{
    ...
    case $ARCH in
        x86) toolchaindir=x86/i686-linux-android-$targetgccversion/bin
            ;;
        arm) toolchaindir=arm/linaro-4.9.1/bin
            ;;
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${axxion_DEVICES_ONLY}" != "z" ]; then
...
{
    target=$1
    axxion_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/axxion/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the axxion model name
            lunch axxion_$target-userdebug
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var axxion_VERSION)
        ZIPFILE=axxion-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.axxion.device=$axxion_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $axxion_BUILD, run away!"
    fi
}
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/axxion/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
